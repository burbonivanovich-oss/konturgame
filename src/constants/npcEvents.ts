...
export const NPC_EVENTS: EventTemplate[] = [
...

  {
    ...
    description: 'Михаил Власов звонит неожиданно: "У меня освободилась партия — другой покупатель отказался в последний момент. Могу отдать вам со скидкой 18%, но нужен ответ сегодня. Кстати, завезу — там конфет захвачу, у племяшки день рождения был, остались." Предложение выглядит честным, но торопит.',
...

  {
    ...
    description: 'Михаил заходит ненадолго — кладёт на прилавок горсть леденцов, как всегда, без объяснений. Говорит тихо: "Слышал, у одного из районных поставщиков проблемы — дважды срывал поставки на этой неделе. Смотрите, у вас случайно нет договора с Аркадием Лапиным?" Он явно намекает, что знает больше, чем говорит.',
...

  {
    ...
    description: 'Анна заходит с конвертом. Стула не берёт — стоит у входа, будто зашла на две минуты. Внутри конверта — проект листовки: "Два магазина, одна улица — скидки на всё". Смотрит прямо: "Перед праздниками все выигрывают от трафика. Пополам расходы, пополам выгода. Рискнём?" Уже держит телефон — ждёт, что вы ответите сейчас.',
...

  {
    ...
    description: 'Вы видите, как Анна на улице разговаривает с Ниной Сергеевной — вашей постоянной покупательницей. Стоит, не присела — раздаёт листовки, показывает что-то на телефоне, говорит быстро. Нина слушает с интересом.',
