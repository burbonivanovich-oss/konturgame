...
export const WEEKLY_TACTICS: WeeklyTacticDef[] = [
  {
    ...
    blurb: '+18% выручки за неделю. Цена за неделю: −9 энергии, −1.4 репутации, −1 лояльности. Solo без сотрудников = выгорание к W14-16.',
  ...
  },
  {
    ...
    blurb: 'Отдых вместо роста: −3% выручки, +10 энергии и +0.7 репутации за неделю. Лекарство после agg-недели.',
    // Спринт 6: +1.5/день = +10.5/нед (раньше было +2.5/день = +17.5/нед —
    // user'у показалось «слишком много восстанавливает», calm одной
    // недели хватало чтобы догнать с 0 до 100). Теперь восстановление
    // даёт примерно недельный буфер, а полный recovery после agg-серии
    // занимает 2-3 недели — это делает выбор тактики реальным.
  ...
  },
  {
    ...
    blurb: 'Без штрафа к выручке. +2 энергии, +5.6 репутации, +3.5 лояльности за неделю. Главный режим для накопления.',
