 ...
 *
 *   1) На W5-6 (после mikhail_crisis W3-5, до svetlana_intro W12+) срабатывает
 ...
 *
 *   3) Светлана появляется ОТДЕЛЬНЫМ сюжетным чейном на W12+ (svetlana_intro),
 *      но только если у игрока уже есть хотя бы один сотрудник. Она —
 *      управленец-выпускница (position 'manager'), усиливает существующую
 *      команду на +25%. Если её отвергнуть — через 3 недели она объявляется
 *      у Анны (svetlana_to_anna).
 */
...
export const FIRST_HIRE_OPTIONS: EventTemplate = {
  ...
  options: [
    ...
    {
      id: 'hire_friend',
      text: '👨 Олег, 39, без энтузиазма: 38 000 ₽/мес, на старте 1.0, но скатится до 0.9 · −6 энергии/нед' + SALARY_NOTE_SUFFIX,
      consequences: {
        balanceDelta: -4500,
        loyaltyDelta: 0,
        hireEmployee: {
 ...
 *
 * Сюжетный чейн svetlana_intro, запускается автоматически с W12+ если
 * игрок уже нанял хотя бы одного сотрудника (гейт в weekCalculator).
 *
...
export const SVETLANA_INTRO: EventTemplate = {
  ...
  options: [
    {
      id: 'hire',
      text: '⭐ Принять — талант не часто стучится. 45 000 ₽/мес, управленец, +25% к команде · −4 энергии/нед' + SALARY_NOTE_SUFFIX,
      consequences: {
        balanceDelta: -6000,
        loyaltyDelta: 6,
        reputationDelta: 2,
      ...
      npcRelationshipDelta: 10,  // принял её — очень благодарна
    },
    {
      id: 'pass',
