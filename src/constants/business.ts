...
export const SERVICES_CONFIG: Record<ServiceType, ServiceConfig> = {
  market: {
    ...
    // Прямые эффекты: пропускная (быстрая касса), +15% к чеку (допродажи и
    // аналитика спроса) и защита от списаний (точный учёт). Снижение списаний
    // реализовано через EXPIRY_LOSS_RATE_WITH_MARKET (0.8→0.4 = -50%), а не
    // через поле writeOffReduction — поэтому здесь его нет (было dead config).
    description: 'Автоматизация торговли: учёт товаров, ЕГАИС, маркировка. +20% к пропускной способности кассы, +15% к среднему чеку (допродажи и аналитика спроса), -50% потерь на просрочке.',
    annualPrice: 24000,
    effects: {
      capacityBonus: 0.2,
      // Спринт 7 (Economy #2): +15% к чеку был захардкожен в economyEngine,
      // скрыт от игрока и нарушал «источник правды». Вынесен в конфиг.
      checkBonus: 0.15,
    },
  },
  bank: {
    ...
    effects: {
      ...
      acquiringRate: 0.010,
      // energyReduction удалён в Спринт 7: поле было dead config (нигде не
      // применялось) и не обещано в описании. Энергию экономит Эльба.
    },
  },
  ofd: {
    id: 'ofd',
    ...
    // запрещено). Ценность — соответствие закону и защита от штрафов ФНС.
    // taxSaving убран в Спринт 7: поле было dead config (в движке применяется
    // только taxSaving Экстерна и синергий), а описание обещало -0.5% — врало.
    description: 'Оператор фискальных данных по 54-ФЗ: онлайн-передача чеков в ФНС. Обязателен для работы с кассой. Защищает от штрафов налоговой за нарушения кассовой дисциплины.',
    annualPrice: 12000,
    effects: {},
  },
  diadoc: {
    ...
    // дней. Клиентов это не привлекает — но ускоряет оборот и экономит на
    // закупках (быстрые отгрузки, скидки за ЭДО, ноль бумаги/курьеров).
    // Спринт 7 (Economy #5): был dead taxSaving 0.005 (как у ОФД, но вдвое
    // дороже — «худший ROI в игре»). Заменён на реальный procurementDiscount.
    description: 'Электронный документооборот: подписание документов с поставщиками за минуты вместо дней. Ускорение оборота снижает закупочные расходы на 5%. Защищает от штрафов за бумажный документооборот.',
    annualPrice: 24000,
    effects: {
      procurementDiscount: 0.05,
    },
  },
  fokus: {
    id: 'fokus',
  ...
  elba: {
    ...
    // отчётность. Реальная ценность — экономия времени на бухгалтерии.
    // Спринт 7: taxSaving убран (dead config — врал про -0.5%), а energyReduction
    // теперь реально применяется в weekCalculator (раньше тоже был dead).
    description: 'Онлайн-бухгалтерия: расчёт УСН, отчётность ФНС/ПФР, кадровый учёт. Экономит время предпринимателя: -35% энергозатрат на операции. Защищает от штрафов за ошибки в декларации.',
    annualPrice: 36000,
    effects: {
