 ...
 *   W11-20:  линейный rampup от 0.65 → 0.95
 *   W21+:    ×0.95 — почти полная стоимость
 *
 * Спринт 5d: снижен с 1.02 до 0.95 — личные цели в районе 1М ₽ требуют
 * накопления существенного остатка к W36-46. При прежнем ×1.02 даже PRO
 * стратегия (все сервисы + апгрейды) eле дотягивала до 1.2М к концу года.
 * 7% скидка на фиксированные расходы во второй половине года освобождает
 * ~5-8К/нед, что превращается в дополнительные 150-250К к финалу.
 */
export function getExpenseMultiplier(currentWeek: number): number {
  if (currentWeek <= 10) return 0.65
  if (currentWeek >= 21) return 0.95
  // 11..20: линейная интерполяция 0.65 → 0.95
