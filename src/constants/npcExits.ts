...
function achievedClosure(
  ...
  switch (goal.id) {
    ...
    case 'own_apartment':
      return {
        title: 'Ключи в руке',
        text: 'Сделка прошла в конце апреля. Подписал у нотариуса, ипотечный договор принёс домой, родителям положил на стол. Мама долго смотрела на бумаги, потом сказала только: «Ну вот». Папа кивнул. Через неделю заехали со старой Икеа-кроватью и одной коробкой — больше пока нет. Первое утро в своей квартире — ничего особенного, только тишина другая.',
      }
  }
}
...
function missedClosure(
  ...
  switch (goal.id) {
    ...
    case 'own_apartment':
      return {
        title: 'Студия уехала',
        text: 'В марте риелтор написала: «Студию купили другие, спасибо за интерес». В мае мама на кухне завела разговор — без укора, просто: «Может, посмотришь у нас на районе? Тут спокойнее». Кивнули. Дома хорошо, конечно. Но «дома у родителей» и «дома у себя» — разные «дома».',
      }
  }
}
