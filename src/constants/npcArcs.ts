...
export const NPC_ARC_EVENTS: EventTemplate[] = [
  ...
  // Сквозной персонаж — внучка Настя, 6 лет. Через неё Тамара
  // привязана к любому типу бизнеса: в магазине покупает молоко с
  // Настей, в кафе — берёт булочку «как в детстве», в салоне
  // приводит Настю на стрижку, сама ждёт у входа. Чтобы это работало
  // без диссонанса с салоном, MEET разделён на 3 версии по бизнесу.
  // ──────────────────────────────────────────────────────────────────
  {
    id: 'NPC_TAMARA_MEET_SHOP',
    title: 'Тамара представилась',
    description:
      'На пятый день к тебе зашла маленькая, в платочке, бабушка. С ней — внучка лет шести, держится за руку. Купила пакет молока. У кассы остановилась: «Меня Тамара зовут, а это Настя. Я тут до вас 30 лет хожу — здесь раньше были «Овощи». За молоком теперь к вам. Если что — спрашивай, я район как пальцы свои знаю». Настя смотрит снизу вверх, серьёзно.',
    trigger: { dayMin: 21, dayMax: 49, randomChance: 1.0, oneTime: true, businessTypes: ['shop'] },
    npcId: 'tamara',
    options: [
      {
        id: 'warm_greet',
        text: 'Тепло поговорить, представиться, кивнуть Насте',
        consequences: { reputationDelta: 2 },
        npcRelationshipDelta: 12,
      },
      {
  ...
  {
    id: 'NPC_TAMARA_MEET_CAFE',
    title: 'Тамара представилась',
    description:
      'Бабушка зашла с внучкой лет шести. Села у окна, заказала чай и одну булочку — корицы. Откусила, прикрыла глаза: «Боже, как в детстве. У нас в столовой такие пекли». Подозвала тебя: «Меня Тамара зовут, а это Настя. У вас булочка — настоящая. Будем теперь ходить». Настя молча ест с серьёзным видом.',
    trigger: { dayMin: 21, dayMax: 49, randomChance: 1.0, oneTime: true, businessTypes: ['cafe'] },
    npcId: 'tamara',
    options: [
      {
        id: 'warm_greet',
        text: 'Сесть рядом, поговорить про детство',
        consequences: { reputationDelta: 2 },
        npcRelationshipDelta: 12,
      },
      {
        id: 'polite_busy',
        text: 'Поблагодарить за добрые слова и вернуться к делам',
        consequences: {},
        npcRelationshipDelta: 0,
      },
    ],
  ...
  {
    id: 'NPC_TAMARA_MEET_SALON',
    title: 'Тамара представилась',
    description:
      'Дверь открылась — и зашла невысокая бабушка в платочке, с внучкой за руку. Девочке лет шесть, чёлка до глаз. «Здравствуйте. Меня Тамара зовут, а это Настя. Нам бы её подстричь. Я слышала, у вас тут с детьми терпеливо работают. Я подожду у окошка, не помешаю?» Села на банкетку, достала вязание.',
    trigger: { dayMin: 21, dayMax: 49, randomChance: 1.0, oneTime: true, businessTypes: ['beauty-salon'] },
    npcId: 'tamara',
    options: [
      {
        id: 'warm_greet',
        text: 'Принять Настю как почётного клиента, налить бабушке чай',
        consequences: { reputationDelta: 2 },
        npcRelationshipDelta: 12,
      },
      {
        id: 'polite_busy',
        text: 'Молча провести Настю к мастеру — обычный клиент',
        consequences: {},
        npcRelationshipDelta: 0,
      },
    ],
  },
  {
    ...
    description:
      'Тамара не приходит уже шесть дней. Без неё и Насти как-то пусто. Спрашиваешь у соседки — лежит дома, плохо себя чувствует, Настю забрала к себе мама. Никого не зовёт. Можно зайти. Можно не лезть. Это её жизнь.',
    trigger: { dayMin: 140, dayMax: 175, randomChance: 1.0, oneTime: true, requiresNpcRevealed: true, npcRelationshipMin: 50 },
    npcId: 'tamara',
    options: [
      {
        id: 'visit',
        text: 'Зайти с гостинцами (−1 500 ₽)',
        consequences: { balanceDelta: -1500, reputationDelta: 5, loyaltyDelta: 3 },
        npcRelationshipDelta: 18,
      },
      {
        id: 'send_box',
        text: 'Передать гостинец через соседку (−800 ₽)',
  ...
  },
  {
    ...
    description:
      'Тамара пришла с Настей, с пакетом — последний раз. «Дочка забирает к себе насовсем. Настю тоже — там школа хорошая. Я сюда — попрощаться. Знаешь, я тут полжизни прожила. Ваше место — последнее, к которому я привыкла. Не закрывайте, ладно?» Достала из пакета старый деревянный календарь. «Это вам. Настя нарисовала на нём ёлку — пусть будет».',
    trigger: { dayMin: 245, dayMax: 315, randomChance: 1.0, oneTime: true, requiresNpcRevealed: true, npcRelationshipMin: 50 },
    npcId: 'tamara',
    options: [
      {
        id: 'farewell_gift',
        text: 'Сделать прощальный подарок Насте (−2 500 ₽)',
  ...
  // ──────────────────────────────────────────────────────────────────
  // GENA — дядя со схемами. Комический рефрен, не классическая арка:
  // знакомство ниже — единственное «meet» событие в арке. Дальше Гена
  // приходит снова и снова из npcEvents.ts с разными темами (крипта,
  // НФТ, бинарные опционы и т.д.) — каждая со своим маленьким шансом
  // выстрелить и пробить джекпот в 500-700 тыс.
  // ──────────────────────────────────────────────────────────────────
  {
    id: 'NPC_GENA_MEET',
    title: 'Дядя Гена с первой темой',
    description:
      'Дядя Гена нашёл тебя на третий день — в дорогущей куртке, которая стоила, наверное, треть того, что ему уже одалживала родня. «Слушай. Криптомат в углу твоего магазина. Обороты в три раза. Я уже договорился с поставщиком — нужно 50 тысяч. К концу месяца верну с двойной». Сейчас откажешь — ничего не случится. Гена будет приходить ещё. С новыми темами.',
    ...
    options: [
      ...
      {
        id: 'curious',
        text: 'Послушать до конца — пусть выговорится',
        consequences: {},
        npcRelationshipDelta: 4,
      },
      {
        id: 'small_invest',
        text: 'Скинуть 5 000 ₽ — лишь бы отстал ненадолго',
